...
def add_mirrored_blur(img, dx, dy):
    try:
        ...
        right = img.crop((img.width - dx, 0, img.width, img.height)).transpose(
            Image.FLIP_LEFT_RIGHT
        )
        top = img.crop((0, 0, img.width, dy)).transpose(Image.FLIP_TOP_BOTTOM)
        bottom = img.crop((0, img.height - dy, img.width, img.height)).transpose(
...
def add_text_to_image(img, text, font_size):
    try:
        ...
        # Starting position
        y_text = (
            img.height - len(lines) * font_size - 20
